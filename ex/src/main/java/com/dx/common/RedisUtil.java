...
public class RedisUtil
{
	...
	public  boolean lock(String key) {
		final boolean result = Boolean.TRUE.equals(redisTemplate.opsForValue().setIfAbsent(key, null, 30, TimeUnit.SECONDS));
		if (result) {
			log.info("[redisTemplate redis]设置锁缓存 缓存  url:{} ========缓存时间为{}秒", key, 30);
		}
		return result;
	}
