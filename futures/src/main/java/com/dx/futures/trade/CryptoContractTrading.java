...
//        this.redisTemplate = redisTemplate;
    ...
    private void matchOrders(String symbol) {
        while (true) {
            ...
            if (buyOrder.isSystemTradable() && sellOrder.isSystemTradable()) {
                System.out.println("系统订单不产生交易");
                break;
            }
    ...
    public Map<Double, Double> getOrderBookSnapshot(String symbol, double[] priceIntervals) {
        ...
        for (int i = 0; i < priceIntervals.length; i++) {
            double interval = priceIntervals[i];
            double intervalend = 0;
            if (i ==priceIntervals.length-1){
                intervalend = Double.MAX_VALUE;
            }else{
                intervalend = priceIntervals[i+1];

            }
            double volume = 0.0;

//            System.out.println(interval+"->"+intervalend);
            for (Map.Entry<Double, Double> entry : orderBook.subMap( intervalend, true, interval, false).entrySet()) {
    ...
    private Order getOrderFromRedis(String orderId) {
        ...
        Order o = convertMapToOrder(orderMap);
